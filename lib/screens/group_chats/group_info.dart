import 'package:insta2/screens/group_chats/add_members.dart';
...
class _GroupInfoState extends State<GroupInfo> {
  ...
  try {
    ...
    // 1. Xóa tất cả id của group khỏi collection 'users' của tất cả các người dùng trong 'membersList'
    ...
    // 2. Xóa collection 'chats' của group
    await _firestore.collection('groups').doc(widget.groupId).collection('chats').get().then((chats) {
      for (var chat in chats.docs) {
        chat.reference.delete();
      }
    });

    // Xóa dữ liệu trong collection 'groups'
    await _firestore.collection('groups').doc(widget.groupId).delete();

    // Xóa thông tin nhóm khỏi collection 'users'
    await _firestore
        .collection('users')
        .doc(_auth.currentUser!.uid)
        .collection('groups')
